...
@push('styles')
<style>
.bn-page{background:#f5f7fb;color:#172033;margin:-24px -24px 0;min-height:100vh;padding:22px 24px 70px}.bn-inner{max-width:1500px;margin:0 auto}.bn-head{align-items:flex-start;display:flex;gap:16px;justify-content:space-between;margin-bottom:16px}.bn-title small{color:#0f8a7a;font-size:13px;font-weight:900;text-transform:uppercase}.bn-title h4{color:#111827;font-size:27px;font-weight:900;line-height:1.2;margin:6px 0}.bn-title p{color:#667085;font-size:14px;margin:0}.bn-actions{display:flex;gap:9px;justify-content:flex-end}.bn-btn{align-items:center;background:#fff;border:1px solid #d0d5dd;border-radius:7px;color:#111827;display:inline-flex;font-size:13px;font-weight:900;gap:8px;justify-content:center;min-height:38px;padding:0 13px;text-decoration:none;white-space:nowrap}.bn-btn.primary{background:#0f8a7a;border-color:#0f8a7a;color:#fff}.bn-btn:hover{filter:brightness(.98);color:inherit}.bn-btn.primary:hover{color:#fff}
...
.bn-list-head,.bn-row{display:grid;gap:12px;grid-template-columns:164px minmax(220px,1fr) minmax(150px,.75fr) minmax(110px,.55fr) minmax(145px,.65fr) 92px;align-items:center}.bn-list-head{background:#fff;border-bottom:1px solid #e4e7ec;color:#667085;font-size:11px;font-weight:900;letter-spacing:.03em;padding:10px 14px;text-transform:uppercase}.bn-row{border-bottom:1px solid #f1f5f9;color:#344054;font-size:13px;min-height:94px;padding:12px 14px}.bn-row:hover{background:#fafafa}.bn-img{background:#f8fafc;border:1px solid #e4e7ec;border-radius:8px;height:70px;object-fit:cover;width:150px}.bn-name{color:#111827;font-size:13px;font-weight:900;line-height:1.35}.bn-sub{color:#667085;font-size:12px;line-height:1.45;margin-top:3px}.bn-cell{min-width:0}.bn-clip{overflow:hidden;text-overflow:ellipsis;white-space:nowrap}.bn-badge{border-radius:999px;display:inline-flex;font-size:12px;font-weight:900;min-height:25px;padding:4px 9px;white-space:nowrap}.bn-badge.success{background:#dcfce7;color:#166534}.bn-badge.muted{background:#f3f4f6;color:#4b5563}.bn-number{color:#111827;font-weight:900;white-space:nowrap}.bn-actions-row{display:flex;gap:6px;justify-content:flex-end}.bn-actions-row form{margin:0}.bn-icon-btn{align-items:center;background:#fff;border:1px solid #d0d5dd;border-radius:7px;color:#111827;display:inline-flex;height:34px;justify-content:center;width:36px}.bn-icon-btn.primary{border-color:#bfdbfe;color:#1d4ed8}.bn-icon-btn.danger{border-color:#fecaca;color:#991b1b}.bn-empty{color:#667085;padding:32px 12px;text-align:center}.bn-pagination{align-items:center;display:flex;gap:10px;justify-content:space-between;padding:13px 14px}.bn-page-info{color:#667085;font-size:12px;font-weight:800}.bn-pager{display:flex;gap:6px;justify-content:flex-end}.bn-page-link{align-items:center;background:#fff;border:1px solid #d0d5dd;border-radius:6px;color:#344054;display:inline-flex;font-size:12px;font-weight:900;height:32px;justify-content:center;min-width:32px;padding:0 10px;text-decoration:none}.bn-page-link.active{background:#0f8a7a;border-color:#0f8a7a;color:#fff}.bn-page-link.disabled{background:#f8fafc;color:#98a2b3;pointer-events:none}
@media(max-width:1180px){.bn-summary{grid-template-columns:repeat(2,minmax(0,1fr))}.bn-filter{grid-template-columns:repeat(2,minmax(0,1fr))}.bn-filter .bn-actions{grid-column:1/-1}.bn-list-head{display:none}.bn-row{border:1px solid #eef2f6;border-radius:8px;grid-template-columns:160px 1fr auto;margin:10px 12px;min-height:auto}.bn-actions-row{align-self:start;grid-column:3;grid-row:1/span 4}}@media(max-width:760px){.bn-page{margin:-24px -12px 0;padding:16px 12px}.bn-head{flex-direction:column}.bn-actions,.bn-btn{width:100%}.bn-summary,.bn-filter{grid-template-columns:1fr}.bn-row{grid-template-columns:1fr}.bn-img{width:100%;height:auto;aspect-ratio:16/7}.bn-actions-row{grid-column:1;grid-row:auto;justify-content:flex-start}.bn-pagination{align-items:flex-start;flex-direction:column}}
</style>
@endpush
    ...
    <div class="bn-inner">
        <div class="bn-head">
            <div class="bn-title">
                ...
                <p>Theo dõi banner trang chủ, banner danh mục và trạng thái hiển thị.</p>
            </div>
            <div class="bn-actions">
                <a class="bn-btn" href="{{ route('home') }}" target="_blank"><i class="fa fa-external-link-alt"></i> Xem website</a>
        ...
        <div class="bn-card">
            ...
            <div class="bn-list">
                ...
                @forelse ($banners as $banner)
                    @php([$label, $class] = $statusBadge($banner->status))
                    <div class="bn-row">
                        ...
                        <div class="bn-cell">
                            ...
                            <div class="bn-sub">Ảnh: {{ $banner->image_url }}</div>
                        </div>
                        <div class="bn-cell">
                        ...
                        <div class="bn-actions-row">
                            <a class="bn-icon-btn primary" href="{{ route('admin.banners.edit', $banner) }}" title="Sửa"><i class="fa fa-edit"></i></a>
                            <form method="post" action="{{ route('admin.banners.hidden', $banner) }}" onsubmit="return confirm('Ẩn banner này?')">
                                @csrf
                                @method('PATCH')
                                <button class="bn-icon-btn danger" title="Ẩn"><i class="fa fa-eye-slash"></i></button>
                            </form>
                        </div>
                    </div>
                @empty
